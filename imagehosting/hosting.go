...
//
// 支持 6 种图床后端，按配置顺序依次尝试，第一个成功的返回结果：
//  1. COS (腾讯云对象存储)
//  2. Bilibili (B站图床)
...
func httpPost(url, contentType string, body io.Reader, header map[string]string) (*http.Response, error) {
	...
	req.Header.Set("User-Agent", "Gensokyo-NewQQ/1.0")
...
// readClose 读取并关闭响应体，同时限制第三方响应体大小。
func readClose(resp *http.Response) ([]byte, error) {
	...
	if len(body) > maxResponseBodyBytes {
		return nil, fmt.Errorf("图床响应体超过限制")
	}
	return body, nil
}
